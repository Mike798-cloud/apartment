(function(){
  ...
  const READING_KEYS={
    ...
    'A栋 1403':['2024 年完成格局优化的 52㎡ 两室一厅','A栋 1403','2026-08','东向采光面','52㎡ · 两室一厅 · 东向','房源编号 QS-A-1403','预计可入住 2026-09-05','A栋 14 层 / 共17层','建筑使用面积约 52㎡','主要采光面向东','2024 年完成格局优化','独立电表、燃气表','厨卫排风接入楼栋公共竖井','厨卫排风为公寓原有公共系统的一部分','2024-06 完成格局优化','2026-08 完成交付前保洁','A栋高区公共排风维护','本页面内容最后更新于 2026-08-18'],
    '户型与楼层导览':['A栋 14 层 · 当前导览','1401、1402、1403、1405、1406','五个住宅单元','当前住户导览示意 · 2024-06-10 更新','个别楼层可能存在尾号不连续的情况','备案图会保留设备/储藏等非住宅空间'],
    '失物招领':['LF231103 · 2023-11-03','A栋 1404 门前','透明伞面，深灰弯柄，绿色束带'],
    '2021 邻里摄影日':['社区活动 · 2021-05-22','A栋活动合影 · 原图经住户授权公开','A栋高层走廊','多扇住户门'],
    '2022 冬季公共区域保洁':['社区维护 · 2022-12-09','A栋高层走廊 · 保洁工作记录','公共镜面','走廊另一侧局部'],
    '六月窗景 · 住户作品选':['住户作品 · 2023-06-24','《高架尽头》 · 周芷珩 · A栋住户','作品拍摄于 2023 年 6 月','远处高架灯光','透明长柄伞'],
    ...
    '旧宿舍更新完成':['社区更新 · 2018-10-02','2018 年夏 · 外立面与公共管线施工阶段','公共排风','衡达工程 · 现场工程协调冯启元','公开文件署名“冯工”','本页面内容最后更新于 2023-12-19'],
    '社区影像资料整理志愿者致谢':['社区新闻 · 2024-04-12','周岚','2023 住户作品授权与历史资料核对','zhoulan.archive@linchuanmail.example','联系电话尾号 8216','本页面内容最后更新于 2024-04-12'],
  ...
  function setupSharedLinkIntro(){
    ...
    document.querySelectorAll('[data-shared-reopen]').forEach(btn=>btn.addEventListener('click',e=>{
      if(box){e.preventDefault();openBox()}
    }));
  }
  ...
  function markPhrase(root,phrase){
    ...
    while(guard++<80){
      ...
      const em=document.createElement('em');em.className='reading-key';em.textContent=phrase;frag.append(em);
  ...
  function setupReadingGuide(){
    if(storageGet(sessionStorage,'qs_reading_guide_v15')==='1')return;const header=document.querySelector('.site-header');if(!header)return;
    const bar=document.createElement('div');bar.className='reading-guide';bar.innerHTML='<div class="wrap"><p><em>阅读辅助已开启：</em> 所有可用于判断与交叉核对的有效信息统一显示为 <em class="reading-key">斜体</em>；斜体不代表答案，仍需结合不同页面判断。</p><button type="button" aria-label="关闭阅读辅助说明">知道了</button></div>';
    header.insertAdjacentElement('afterend',bar);bar.querySelector('button').addEventListener('click',()=>{storageSet(sessionStorage,'qs_reading_guide_v15','1');bar.remove()});
  }
