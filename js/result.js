(function(){
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    box.innerHTML='';
    if(!s.ending||!outcomes[s.ending]||!s.finalChoice||!s.finalChoice.action||endingByAction[s.finalChoice.action]!==s.ending){
    ...
    if(window.QishanState&&typeof QishanState.assessEvidence==='function'){
      const rule=QishanState.assessEvidence().options?.[s.finalChoice.action];
      if(rule&&rule.allowed===false){
        const h=document.createElement('h2');h.textContent='这条回复还缺少依据';
        const p=document.createElement('p');p.textContent=rule.reason+' 先回去继续核对公开资料，再决定是否这样回复许唯。';
        box.append(h,p);return;
      }
    }
